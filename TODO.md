...
Fichier download.yml et install.yml supprimé pour ne garder que windows.yml & linux.yml
...
On peut lire cela a la ligne 100 du template pour Windows par exemple.

Explication : pourquoi graylog est dans les templates de conf : 

Au tout début du projet les logs étaient affichés sous forme de message complet. Il était très difficile de récupérer les champs interessants ne serai-ce que un utilisateur ou un code d'erreur. Après avoir choisi JSON comme format, Grafana recevait mal les logs et ne voulait pas les afficher. 
